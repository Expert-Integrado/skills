...
- **Env `SUPABASE_PAT`** (obrigatória pro script; `SUPABASE_SERVICE_ROLE` NÃO é necessária pra esta skill). Fonte canônica: 1Password, vault `Agentes Eric`. Cache local em `~/.claude.json` após `setup-secrets.ps1`; na VPS (container claude-code) já vem exportada pelo `boot-tmux.sh`. Como o valor é sensível, NUNCA imprimir o token — checar só a PRESENÇA (ver passo 2 pro comando exato que não vaza o valor).
...
```bash
export SUPABASE_PAT=$(op read "op://Agentes Eric/SUPABASE_PAT/credential") && python3 "${CLAUDE_PLUGIN_ROOT}/skills/estou-devendo/scripts/estou_devendo.py" $ARGUMENTS
```
