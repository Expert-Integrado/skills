...
**IMPORTANTE — de onde `_load_creds()` le (fato do codigo, corrige suposicao comum):** dentro da engine o caminho da pasta `claude-sync` esta HARDCODED como `C:/Users/Eric Luciano/OneDrive/Workspace/claude-sync` (constante `SYNC` no topo do arquivo). `_load_creds()` **NAO** respeita a env var `WORKSPACE_DIR` — `WORKSPACE_DIR` so afeta onde o SEU runner localiza o arquivo `.py` pra importar (passo 4), nao de onde a engine le os secrets. Consequencia pratica: em maquina onde a `claude-sync` nao esteja exatamente nesse caminho (notebook com outro usuario, VPS Linux), `_load_creds()` quebra com `FileNotFoundError` — e a correcao e no arquivo canonico (tarefa fora desta skill), nao um workaround inline. No PC do Eric o caminho bate e funciona.
...
2. **Onde voce esta rodando (confirmar o ambiente antes de assumir o path):** esta skill so roda de fato no **PC do Eric (Windows)**, porque a engine tem o caminho da `claude-sync` HARDCODED (ver "IMPORTANTE" no CONTRATO DA ENGINE). Como confirmar, sem adivinhar: rodar `ls "$HOME/OneDrive/Workspace/claude-sync/scripts/whatsapp-central-prospeccao-batch.py"` (ou o caminho de `WORKSPACE_DIR` se setado).
   - SE o arquivo existe → ambiente OK, seguir.
   - SE `$HOME` NAO resolver pro perfil certo (falso-negativo comum no Windows: em alguns shells `$HOME` aponta pra outro lugar que nao a pasta do usuario, ou nem esta setado) → ANTES de concluir que nao e o PC do Eric, testar tambem `ls "$USERPROFILE/OneDrive/Workspace/claude-sync/scripts/whatsapp-central-prospeccao-batch.py"`. Se qualquer um dos dois achar o arquivo → ambiente OK, seguir.
   - SE NENHUM dos dois (`$HOME` e `$USERPROFILE`) achar o arquivo → voce NAO esta no PC do Eric (pode ser notebook/VPS). PARAR e reportar ao usuario: "esta campanha so roda no PC do Eric (a engine tem path fixo de credenciais); confirme que estamos no PC certo ou rode la". NAO tentar workaround de path.
3. **WORKSPACE_DIR — pra que serve exatamente (nao confundir):** `WORKSPACE_DIR` (default PC: `$HOME/OneDrive/Workspace`; em Python `os.environ.get('WORKSPACE_DIR', os.path.expanduser('~/OneDrive/Workspace'))`) serve SO pra o SEU runner localizar e importar o arquivo `.py` da engine. Ele **nao** muda de onde a engine le credenciais — isso e hardcoded dentro dela. Ou seja: mesmo setando `WORKSPACE_DIR`, se a `claude-sync` real nao estiver em `C:/Users/Eric Luciano/OneDrive/Workspace/claude-sync`, `_load_creds()` quebra. Por isso o passo 2 acima e obrigatorio.
...
5. **Credenciais** — a engine le sozinha (`_load_creds`, sem argumentos) dos JSONs locais no path hardcoded `C:/Users/Eric Luciano/OneDrive/Workspace/claude-sync`; validar que existem ANTES de rodar (Read/ls nesses dois arquivos):
   - `{WORKSPACE_DIR}/claude-sync/claude_desktop_config.json` → `mcpServers.pipedrive.env.PIPEDRIVE_API_KEY`
   - `{WORKSPACE_DIR}/claude-sync/claude_desktop_config-ERICLUCIANO-PC.json` → `mcpServers.chatguru-mcp.env.{CHATGURU_API_KEY, CHATGURU_ACCOUNT_ID, CHATGURU_PHONE_ID}`
   - Se algum JSON faltar → rodar `setup-secrets.ps1` (PC) ou reportar ao usuario e parar. Fonte canonica dos secrets e o 1Password (vault "Agentes Eric", via `op read "op://Agentes Eric/<TOKEN>/credential"`); os JSONs sao cache legado que a engine ainda le — migrar a engine pro `op` e pendencia do script, fora desta skill.
...
WORKSPACE = os.environ.get('WORKSPACE_DIR', os.path.expanduser('~/OneDrive/Workspace'))
