...
- **NUNCA gastar crédito (ElevenLabs/HeyGen/Higgsfield) antes do gate da etapa 2.5 aprovado** com "prosseguir? (s/n)".
...
- NUNCA usar a flag `--image` no Higgsfield: é alias de `--image-references` e o `seedance1_5` só aceita `start_image` — o clipe sai sem o frame. SEMPRE `--start-image` (o script já faz).
- NUNCA rodar lote de B-roll sem checar a franquia (`hf.exe account status`): os 200 créditos/mês são COMPARTILHADOS entre o PC e o notebook do Eric.
...
- SEMPRE banco de B-rolls primeiro; Higgsfield só pros gaps que o banco não cobre.
...
3. **Credenciais** (pasta default do PC: `C:/MCPs/` — os scripts têm esse default chumbado nas constantes do topo; máquina nova = ajustar via `SETUP.md`). Checar existência com `ls`; NUNCA imprimir o conteúdo. Cada `.env` é consumido por uma etapa diferente — abortar conforme o branch, não tudo de cara:
   ...
   - **Higgsfield CLI — CONDICIONAL ao gap de B-roll** (não é `.env`, é binário + login): só é consumido se a etapa 6 achar trechos que o banco NÃO cobre. Reel 100% coberto pelo banco não usa — checar só quando o gap surgir, não abortar de cara. Checagem: `ls C:/MCPs/hf.exe` (override: env `HF_EXE`) e `C:/MCPs/hf.exe account status` — tem que devolver `operacoes@expertintegrado.com.br — starter plan, N credits`. SE der `Not authenticated` → `hf.exe auth login` e **pedir ao Eric o clique em Authorize** (OAuth com PKCE, agente não completa sozinho). SE der `No workspace selected` → `hf.exe workspace set d91a7bf6-e682-47a2-9c61-4a2a6cd6649b`. Runbook: `references/higgsfield-cli.md`.
...
- **Forma preferida (o script calcula o nº de B-rolls sozinho):** `"$PY" scripts/simular_custo.py --cenas-file "$REEL/cenas.txt" --clips-hf 0`
  ...
  - `--clips-hf` = B-rolls que vão pro Higgsfield DEPOIS de checar o banco (etapa 6) — como ainda não sabe, rodar com `0` e REFAZER a simulação após a etapa 6 se houver gaps. (`--clips-kling` ainda funciona como alias depreciado.)
- O script imprime o **custo conhecido** (HeyGen lip-sync + ElevenLabs + imagens) e, se houver gap, os **créditos da franquia** do Higgsfield à parte — B-roll novo é R$ 0 de caixa enquanto couber nos 200/mês (`references/custos.md`). Passar `--creditos-restantes N` com o saldo real (`hf.exe account status`) faz o script avisar se o reel estoura a franquia.
...
> **Ordem REAL de execução do bloco de B-rolls (não é a ordem dos números):** (1) fazer primeiro a **etapa 6** (banco remoto) pra descobrir quais dos N trechos o banco NÃO cobre; (2) SÓ ENTÃO voltar a esta etapa 5 e gerar 1 frame por gap; (3) seguir pros sub-passos do Higgsfield da etapa 6. Se o banco cobrir os N trechos, esta etapa 5 gera apenas a thumb (etapa 8) e a capa (etapa 9).
...
- Validar cada frame com `Read` antes de mandar pro Higgsfield.

### 6. B-rolls — BANCO REMOTO PRIMEIRO, Higgsfield só pro que faltar
- Banco = catálogo público de ~219 clips reutilizáveis em GitHub Release (funciona em qualquer máquina, baixa só o que precisa; cache em `~/.cache/broll-bank/`; override do catálogo: env `BROLL_BANK_URL`). Ver `references/banco-broll.md`.
  ...
  5. **Só gerar no Higgsfield os trechos que o banco NÃO cobre** (visual muito específico) — e só pra esses fazer o frame (etapa 5). Numerar os clips novos preenchendo os buracos da sequência. Isso zera/derruba o consumo da franquia na maioria dos reels (pedido do João).
...
- SE houver gaps (Higgsfield):
  1. Conferir a franquia: `C:/MCPs/hf.exe account status` (200 créditos/mês, renova dia 10, **compartilhada com o notebook**). Refazer a simulação da etapa 2.5 com `--clips-hf <M> --creditos-restantes <saldo>` e reconfirmar com o Eric/João.
  2. `Write` em `$REEL/manifest.json` (formato em `references/higgsfield-cli.md`). `model` e `hf_flags` são OPCIONAIS: sem eles o runner usa `seedance1_5` em 480p/4s sem áudio = **~1,2 crédito/clipe**, o mais barato validado. Só sobrescrever pra caso especial (720p custa ~2x).
  3. Validar 1 clipe antes do lote: `"$PY" scripts/higgsfield_i2v.py manifest.json 1` (background). O runner faz preflight (binário, login, workspace) e imprime o saldo antes de gastar.
  4. Lote: `"$PY" scripts/higgsfield_i2v.py manifest.json 2 3 ...`. Conferir cada clip com `ffprobe` (tem que dar `codec_name=h264` e duração ~4s); SE algum falhar → re-disparar só esses (`... manifest.json 7 8 9`).
- **Kling: fora do fluxo desde 28/07/2026** (resource packs pré-pagos venceram; API responde `1102 Account balance not enough`). O `scripts/kling_i2v.py` e o `references/kling-api.md` ficam no repo como caminho legado — só usar se o Eric comprar pacote novo, o que só compensa acima de ~160 clipes/mês. NÃO tratar como fallback automático: se o Higgsfield falhar, parar e reportar.
...
| `No workspace selected` no Higgsfield | workspace não setado (obrigatório no CLI 1.1.20+) | `hf.exe workspace set d91a7bf6-e682-47a2-9c61-4a2a6cd6649b` |
| `Not authenticated` no Higgsfield | token ausente/expirado | `hf.exe auth login` + **pedir o clique do Eric** em Authorize (PKCE, agente não completa) |
| Franquia Higgsfield no fim | 200 créditos/mês, compartilhada com o notebook | Cobrir mais trechos pelo banco, ou Top Up no site; conferir com `account status` |
| `clip-NN.mp4` é imagem/não abre | `find_url` pegando a URL de ENTRADA (`params.medias`) | Já corrigido (lê `result_url` + valida header `ftyp`). Se voltar, ver `references/higgsfield-cli.md` |
| Clipe sem relação com o frame | flag `--image` (= `--image-references`) em vez de `--start-image` | Usar `--start-image`; o runner já faz |
...
| Submit recusado (HeyGen/Higgsfield) | — | Não gasta crédito; corrigir payload e reenviar |
...
- **`scripts/higgsfield_i2v.py`** — runner do Higgsfield (**TITULAR dos B-rolls**): preflight de login/workspace, `--start-image`, defaults de economia e validação do MP4 baixado.
...
- **`references/higgsfield-cli.md`** — instalação do binário (npm quebra), login/workspace, modelos e preços, manifesto, troubleshooting.
