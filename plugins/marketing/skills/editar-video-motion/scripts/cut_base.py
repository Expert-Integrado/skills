...
def main():
    ...
    ap.add_argument("--keep", action="store_true", help="mantem a pasta _cutwork (debug)")
    a = ap.parse_args()
    src = norm(a.src)
    ...
    if os.path.isdir(work):
        shutil.rmtree(work)   # comeca limpo (sem pedacos de runs antigas)
    os.makedirs(work)
    files = []; t = 0.0
    ...
    for i, (lbl, s, e) in enumerate(pieces):
        ...
        if a.audio:
            ...
            subprocess.run(["ffmpeg", "-y", "-ss", str(s), "-i", src, "-t", f"{dur:.3f}",
                            ...
                            "-c:a", "aac", "-b:a", "192k", "-ar", "48000", "-af", af, outp], capture_output=True)
    ...
    with open(listf, "w") as f:
        if a.audio:
            subprocess.run(["ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=44100:cl=stereo",
                            "-t", str(a.gap), f"{work}/sil.wav"], capture_output=True)
    ...
    if a.audio:
        subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", listf, "-c", "copy", f"{work}/m.wav"], capture_output=True)
        subprocess.run(["ffmpeg", "-y", "-i", f"{work}/m.wav", "-codec:a", "libmp3lame", "-q:a", "2", a.out], capture_output=True)
    else:
        subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", listf, "-c", "copy", a.out], capture_output=True)
