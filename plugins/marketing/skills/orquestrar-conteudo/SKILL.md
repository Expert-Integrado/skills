...
3. **Preview SEMPRE (desktop):** quando o Workflow retorna `aprovado`, a sessão principal copia `assets/gallery-template.html` para `temp/<slug>/index.html` (galeria-**carrossel**: 1 slide por vez, setas ‹ ›, teclado ← →, bolinhas, clique-pra-ampliar; **auto-detecta** o nº de slides). **Tem que ser `index.html`** (não `gallery.html`) — o painel de preview do Claude Code abre na RAIZ do server; sem `index.html` a raiz mostra listagem de arquivos e o Eric não vê o carrossel. Depois sobe um server via `preview_start` (entrada no `.claude/launch.json` servindo `temp/<slug>` numa porta livre, ex. 4530); o preview já renderiza o carrossel na raiz `/`. Headless/VPS (sem preview): pular e apontar o caminho dos PNGs.
